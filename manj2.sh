...
pacman -S cinnamon cinnamon-translations cinnamon-wallpapers networkmanager cinnamon-sounds gnome-terminal parcellite lightdm lightdm-slick-greeter lightdm-settings manjaro-cinnamon-settings adapta-maia-theme kvantum-manjaro pamac pulseaudio pavucontrol && systemctl enable lightdm
	elif [[ "$choice" == "3" ]]; then
pacman -S gnome gnome-extra manjaro-settings-manager pamac networkmanager pulseaudio pavucontrol && systemctl enable gdm
	elif [[ "$choice" == "4" ]]; then
pacman -S xfce4-gtk3 xfce4-goodies xfce4-terminal network-manager-applet networkmanager xfce4-notifyd-gtk3 xfce4-whiskermenu-plugin-gtk3 tumbler pamac engrampa lightdm lightdm-gtk-greeter lightdm-gtk-greeter-settings manjaro-settings-manager pulseaudio pavucontrol && systemctl enable lightdm
	elif [[ "$choice" == "5" ]]; then
pacman -S lxqt xscreensaver lightdm lightdm-slick-greeter network-manager-applet networkmanager lightdm-settings light-locker manjaro-openbox-adapta-maia papirus-maia-icon-theme pamac pulseaudio pavucontrol-qt && systemctl enable lightdm
	elif [[ "$choice" == "6" ]]; then
pacman -S mate network-manager-applet networkmanager mate-extra dconf-editor lightdm lightdm-slick-greeter lightdm-settings arc-maia-icon-theme papirus-maia-icon-theme manjaro-settings-manager manjaro-settings-manager-notifier pamac pulseaudio pavucontrol && systemctl enable lightdm
	fi

...
if [[ "$choice" == "1" ]]; then
    mkfs.btrfs -L Manj -f $DEV_ && mount $DEV_ /mnt && \
    cd /mnt && btrfs sub cre @ && btrfs sub cre @home && \
    btrfs sub cre @cache && btrfs sub cre @log && cd / && umount /mnt
elif [[ "$choice" == "2" ]]; then
    mkfs.ext4 -L Manj $DEV_ && mount $DEV_ /mnt && mkdir /mnt/{data,data2} && \
    mkdir -p /mnt/boot/efi && cd / && umount /mnt
fi
...
echo '1-ЯДРО-5.15-LTS, 2-ЯДРО-5.19'
read choice

if [[ "$choice" == "1" ]]; then
    basestrap /mnt base linux515 mhwd linux-firmware dhcpcd nano
elif [[ "$choice" == "2" ]]; then
    basestrap /mnt base linux519 mhwd linux-firmware dhcpcd nano
fi
